...
{
	[TestClass]
	public class ConcurrentObservableListNotifierTest
	{
		...
		public async Task AddToSourceList()
		{
			...
			// Check that the notifier collection contains as many values as were added.
			Assert.IsTrue(notifierList.Count == two.Count+three.Count+five.Count);

			// Check that all added values are found in the notifier collection.
			Parallel.ForEach(two,	(value)=>Assert.IsTrue(notifierList.Any(currentValue => currentValue.First == value), $"The value {value} wasn't added from the {nameof(two)} list."));
			Parallel.ForEach(three,	(value)=>Assert.IsTrue(notifierList.Any(currentValue => currentValue.First == value), $"The value {value} wasn't added from the {nameof(three)} list."));
			Parallel.ForEach(five,	(value)=>Assert.IsTrue(notifierList.Any(currentValue => currentValue.First == value), $"The value {value} wasn't added from the {nameof(five)} list."));
			...
			foreach(DummyClass value in notifierList)
				Assert.IsTrue(notificationValuesArray.Contains(value.First), $"The value {value} wasn't raised by the {nameof(ConcurrentObservableCollection<DummyClass>.CollectionChanged)} event.");

			// Check that the property notify changed event was called the correct number of times, for the Count property.
			Assert.AreEqual(two.Count+three.Count+five.Count, countChanged, $"The {nameof(ConcurrentObservableListNotifier<DummyClass>.PropertyChanged)} event wasn't raised the expected number of times for the {nameof(ConcurrentObservableListNotifier<DummyClass>.Count)} property.");
			...
			Assert.AreEqual(two.Count+three.Count+five.Count, firstPropertyChanged, $"The {nameof(ConcurrentObservableListNotifier<DummyClass>.CollectionPropertyChanged)} event wasn't raised the expected number of times for the {nameof(DummyClass.First)} property.");
		...
		/// <summary>
		/// Tests that:
		/// CollectionChanged event is raised, every time an item is added to the notifier list.
		/// PropertyChanged event is raised, every time an item is added to the notifier list.
		/// CollectionPropertyChanged event is raised, every time a property is modified in the notifier list.
		/// Multiple threads can add to the list, at the same time.
		/// Multiple threads threads can modify properties in the list, at the same time.
		/// </summary>
		[TestMethod]
		public async Task AddToNotifierList()
		{
			const int										iterations				= 10000;
			ConcurrentObservableList<DummyClass>			list					= new ConcurrentObservableList<DummyClass>();
			ConcurrentObservableListNotifier<DummyClass>	notifierList			= new ConcurrentObservableListNotifier<DummyClass>(list);
			...
			// Add integers divisible by two, to the notifier collection.
			Task twoTask = Task.Run(async ()=>
			{
				Parallel.ForEach(two, (value)=>
				{
			...
			// Add integers divisible by three, to the notifier collection.
			Task threeTask = Task.Run(async ()=>
			{
				Parallel.ForEach(three, (value)=>
				{
			...
			// Add integers divisible by five, to the notifier collection.
			Task fiveTask = Task.Run(async ()=>
			{
				Parallel.ForEach(five, (value)=>
				{
